...
class Apitest2 extends MY_Controller  {    
 ...
 function update_profile2()
     { 
        ...
        if($this->check_accesstoken($user_id,$accesstoken))
        {
            ...
            $total->user_type = $this->Core_Model->selectsinglerecord('users', 'user_type', $where);
            // print_r($total->user_type);die(); 
            if ($total->user_type == 2) 
            {
                $where_update = array('id' => $user_id);            
                $result = $this->Core_Model->updateFields('users', $field_update, $where_update);
            }else
            {
                // $field_update['skills'] = 0;
                // $where_update = array('id' => $user_id);            
                // $result = $this->Core_Model->updateFields('users', $field_update, $where_update);
            }
 ...
 public function signup() 
    {
        ...
        $where = array('name'=>$name,'email'=>$email,'password'=>md5($password),'last_login'=>$last_login,'contact'=>$contact,'user_type'=>$user_type);
        ...
        $array_login = $this->Core_Model->selectsinglerecord('users', '*', $where_login);            
            ...
            $array_login2 = $this->Core_Model->selectsinglerecord('users', 'id,name,email,contact,gender,dob,image,token,is_login,last_login,user_type', $where_login);
            ...
            $result['user_type'] = ($array_login2->user_type) ? $array_login2->user_type : '';
...
//*********************************************************************************************************
   public function viewProposal()
    { 
        ...
        { 
            $where = array('id' => $id);               
            $jresult = $this->Core_Model->selectsinglerecord('job_post', 'status,service_provider_id,proposal_id', $where); 
            // print_r($jresult);
...
//*********************************************************************************************************  
 public function getMyProjects2()
    {
        $request = json_decode(rtrim(file_get_contents('php://input'), "\0"));        
        $user_id = $request->user_id;
        // print_r($id);die();
  ...
  public function changeJobStatus()
    { 
        ...
        {     
            ...
            // print_r($service_provider_id);die();
         ...
         // print_r($result);die();
            // $total = $this->Core_Model->selectsinglerecord('job_post', 'id,user_id,service_id,description,budget,proposal_id,status,date_time', $where_login);
            // print_r($total);die();
            // $total->image = base_url('upload/profile/').$total->image; //image url get code
            // $result = [];
            // $result['id'] = ($total->id) ? $total->id : '';
            ...
            // $result['awarded_user'] = ($total->proposal_id) ? $total->proposal_id : '';
            // // $result['image'] = ($total->image) ? $total->image : '';
    ...
    function jobDetail()
     { 
        ...
        $userdata = [];
            ...
            $pdata = $this->Core_Model->SelectRecord('proposal', 'user_id', $where3, $order = '');
            ...
            {                
                ...
                $vendor_arr[]  =  ['vendor_id'=>(($vtotal['id']) ? $vtotal['id'] : ''),'name'=>(($vtotal['name']) ? $vtotal['name'] : ''),'image' => ($vtotal['image']? base_url('upload/profile/').$vtotal['image'] : ''),'review' => (($dreview['review']) ? $dreview['review'] : ''),'rating' => (($dreview['rating']) ? $dreview['rating'] : '')];
                }
            }   
            $skills = $total['skills'];
            ...
            $user_arr = ['user_id'=>(($total['user_id']) ? $total['user_id'] : ''),'job_post_id'=>(($total['id']) ? $total['id'] : ''),'description' => (($total['description']) ? $total['description'] : ''),'skills'=>$res2 ? rtrim($res2, ','): '','project_budget' => (($total['budget_amount']) ? $total['budget_amount'] : ''),'delivery_address' => (($total['address']) ? $total['address'] : ''),'ends_on' => (($total['date_time']) ?  $datediff.' days' : '0 days')];
            }
...
function jobDetail2()
     { 
        ...
        $job_post_id = $request->job_post_id;
         // print_r($job_post_id);die();
        $this->db->select('proposal.*,users.name,users.image,review.review,review.rating,review.senderId');    
        $this->db->from('proposal');
        $this->db->join('users', 'proposal.user_id = users.id');
        $this->db->join('review', 'proposal.user_id = review.receiverId');
        $this->db->where('proposal.job_post_id',$job_post_id);
        $query=$this->db->get();
        $result1= $query->result_array();
        $userdata = [];
            foreach ($result1 as $key=>$total) 
            {
            // // print_r($total);die();
            $job_post_id = $total['job_post_id'];
            // print_r($job_post_id);
            $where3 = array('job_post_id' => $job_post_id);
            $pdata = $this->Core_Model->SelectRecord('proposal', '*', $where3, $order = '');
            $bid = 0;
            if(count($pdata)){
            $bid = count($pdata);
                }
                
            $userdata[] = Array('user_id' => $total['user_id'],'job_post_id' => $total['job_post_id'],'name' => $total['name'],'image' => base_url('upload/profile/').$total['image'],'review' => $total['review'],'rating' => $total['rating']);
            // die;
            $arr[]  = ['user_id'=>(($total['user_id']) ? $total['user_id'] : ''),'vendor_id'=>(($total['senderId']) ? $total['senderId'] : ''),'job_post_id'=>(($total['job_post_id']) ? $total['job_post_id'] : ''),'name'=>(($total['name']) ? $total['name'] : ''),'image' => ($total['image']? base_url('upload/profile/').$total['image'] : ''),'review' => (($total['review']) ? $total['review'] : ''),'rating' => (($total['rating']) ? $total['rating'] : ''),'proposals'=>$bid];
            }
            // print_r($arr);die();
            if (empty($userdata)) 
            {
...
//*********************************************************************************************************  
  public function browsJobs()
    { 
        ...
        $res = $this->Core_Model->joindataResult($place1 =' job_post.service_id', $place2 = 'category.id',$where='','job_post.user_id,job_post.service_id,job_post.id as jobpostid,job_post.description,job_post.budget_amount,job_post.skills,job_post.status,category.id,category.title','category','job_post','job_post.id DESC');
        // print_r($res);die();
        ...
        {   
            ...
            $arr1[]  = ['id'=>($total['jobpostid']) ? $total['jobpostid'] : '','user_id'=>($total['user_id']) ? $total['user_id'] : '','service_id'=>($total['service_id']) ? $total['service_id'] : '','description'=>($total['description']) ? $total['description'] : '','budget'=>($total['budget_amount']) ? $total['budget_amount'] : 0,'proposals'=>$bid,'skills'=>$res2 ? rtrim($res2, ','): ''];
        } 
        ...
        foreach ($res as $key => $value) {      
            //print_r($value);    
            $reslt = $this->Core_Model->joindataResult($place1 =' job_post.service_id', $place2 = 'category.id',$where=array("job_post.service_id"=>$value['service_id']),'job_post.user_id,job_post.service_id,job_post.id as jobpostid,job_post.description,job_post.budget_amount,job_post.skills,job_post.status,category.id,category.title','category','job_post','job_post.id DESC');
            if($reslt){
                $popular[] = $reslt;
            }
        }
        ...
        foreach ($popular as $poplr){ 
            ...
            {   
                ...
                $arr2[]  = ['id'=>($total['jobpostid']) ? $total['jobpostid'] : '','user_id'=>($total['user_id']) ? $total['user_id'] : '','service_id'=>($total['service_id']) ? $total['service_id'] : '','description'=>($total['description']) ? $total['description'] : '','budget'=>($total['budget_amount']) ? $total['budget_amount'] : 0,'proposals'=>$bid,'skills'=>$res2 ? rtrim($res2, ','): ''];
       
            } 
        }

        $res = $this->Core_Model->joindataResult($place1 =' job_post.service_id', $place2 = 'category.id',$where='','job_post.user_id,job_post.service_id,job_post.id as jobpostid,job_post.description,job_post.budget_amount,job_post.skills,job_post.status,category.id,category.title','category','job_post','job_post.id ASC');
        // print_r($res);die();
        //$skills = explode(",", $res[0]['skills']);        
        ...
        {   
            ...
            $arr3[]  = ['id'=>($total['jobpostid']) ? $total['jobpostid'] : '','user_id'=>($total['user_id']) ? $total['user_id'] : '','service_id'=>($total['service_id']) ? $total['service_id'] : '','description'=>($total['description']) ? $total['description'] : '','budget'=>($total['budget_amount']) ? $total['budget_amount'] : 0,'proposals'=>$bid,'skills'=>$res2 ? rtrim($res2, ','): ''];
   
        }

        $res = $this->Core_Model->joindataResult($place1 =' job_post.service_id', $place2 = 'category.id',$where='','job_post.user_id,job_post.service_id,job_post.id as jobpostid,job_post.description,job_post.budget_amount,job_post.skills,job_post.status,category.id,category.title','category','job_post','job_post.budget_amount DESC');
        // print_r($res);die();
        //$skills = explode(",", $res[0]['skills']);        
        ...
        {   
            ...
            $arr4[]  = ['id'=>($total['jobpostid']) ? $total['jobpostid'] : '','user_id'=>($total['user_id']) ? $total['user_id'] : '','service_id'=>($total['service_id']) ? $total['service_id'] : '','description'=>($total['description']) ? $total['description'] : '','budget'=>($total['budget_amount']) ? $total['budget_amount'] : 0,'proposals'=>$bid,'skills'=>$res2 ? rtrim($res2, ','): ''];
   ...
   public function category() 
    { 
        $categories2 = $this->MY_Model->SelectRecord('category','*',$udata=array("is_deleted"=>"0","parent_id"=>"0"),'order_id asc');            
            ...
            $result_set  = array("categories"=>$arr,"skills"=>$skill_arr); 
            //$arr    = []; 
         }
            if (empty($result_set)) 
            {
    ...
    { 
        ...
        if($this->check_accesstoken($user_id,$accesstoken))
        {
         ...
         // print_r($status);die(); 
            ...
            {
                $this->res->success = 'true';
                $this->res->message = 'Proposal/Bid Posted Successfully';
                // $this->res->data = $result;
            } 
    ...
    public function changePasswordOld()
    {
        ...
        if($this->check_accesstoken($user_id,$accesstoken))
        {
            ...
            if (strlen($password) < 6)                 
            {
                ...
                $array_login2 = $this->Core_Model->selectsinglerecord('users', 'id,name,email,password,contact,gender,dob,image,token,is_login,last_login', $where_user);
                $array_login2->image = base_url('upload/profile/').$array_login2->image; //image url get code
                $result = [];
                $result['id'] = ($array_login2->id) ? $array_login2->id : '';
                $result['name'] = ($array_login2->name) ? $array_login2->name : '';
                $result['email'] = ($array_login2->email) ? $array_login2->email : '';
                $result['password'] = ($array_login2->password) ? $array_login2->password : '';
                $result['contact'] = ($array_login2->contact) ? $array_login2->contact : '';
                $result['gender'] = ($array_login2->gender) ? $array_login2->gender : '';
                $result['dob'] = ($array_login2->dob) ? $array_login2->dob : '';
                $result['image'] = ($array_login2->image) ? $array_login2->image : '';
    ...
    public function getMyProjects()
    {
        ...
        // print_r($id);die();
         ...
         if($this->check_accesstoken($user_id,$accesstoken))
         {
            ...
            //$user_id = 58;
            $where = array('user_id' => $user_id,'is_deleted' => $is_deleted);            
            $res = $this->Core_Model->SelectRecord('job_post','*',$where,'id desc');
            if (empty($res)) {
                $arr2  = array();
                $this->res->success = 'true';
                $this->res->message = 'No projects available';          
                ...
                foreach ($where2 as $value2)
                {
                  ...
                  $bids = $this->Core_Model->SelectRecord('proposal','*',['job_post_id'=>$value['id']],'id desc');
                    $arr[]  = ['id'=>($value['id']) ? $value['id'] : '',
                               'title'=>($value['job_title']) ? $value['job_title'] : '',
                               'description'=>($value['description']) ? $value['description'] : '',
                               'skills'=>$res2 ? rtrim($res2, ','): '',
                               'budget'=>($value['budget_amount']) ? $value['budget_amount'] : '',
                               'ends_on'=>($value['date_time']) ? $datediff : '',
                               'total_bids'=>count($bids),
                               'status'=>($value['status'] == 1) ? 'Open' : (($value['status'] == 2) ? 'Awarded' : 'Closed')
                               ];
                    $this->res->success = 'true';
