...
class Core_Model extends CI_Model {
   ...
   function jointables($place1, $place2, $WhereData, $Selectdata, $TableName1, $TableName2, $orderby,$place3,$TableName3,$place4,$TableName4) {
    ...
    function CountBid($where){
                 $res=$this->db->query("select count(*) as total from proposal where job_post_id= $where3");
    ...
    function joindataResult2($place1, $place2, $WhereData, $Selectdata, $TableName1, $TableName2, $orderby) {
     ...
     function joindataResultCount($place1, $place2, $WhereData, $Selectdata, $TableName1, $TableName2, $orderby) {
        $this->db->select("$Selectdata,COUNT(job_post.service_id), job_post.service_id");
        $this->db->from($TableName1);
        $this->db->join($TableName2, $place1 . '=' . $place2);
        if($orderby!='')
        //$this->db->order_by($orderby);
        $this->db->group_by('job_post.service_id'); 
        if($WhereData!='')
        $this->db->where($WhereData);
        $query = $this->db->get();
        return $query->result_array();
    ...
    function get_popular_service($tbl) {
    ...
    function get_myprojects($tbl,$user_id) {
        $this->db->select("*");
        $this->db->from($tbl); 
        $this->db->where('user_id',$user_id);                        
        $this->db->or_where('service_provider_id',$user_id); 
